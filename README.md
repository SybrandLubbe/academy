...
- **Knowledge Check** is a 10-question quiz at the end of every module, visible to all students — but **answers are not on the module pages at all**. Students see only the questions.
- **Trainer Answer Key** (`trainer.html`) is a separate, PIN-gated page that contains all 220 questions with their full model answers, organised by module, with a quick-jump nav and a Print / PDF button. The default PIN is **`netcentric`** (the page stores the SHA-256 hash of that string — change it there if you want to rotate the PIN). The file is not linked from any student-facing page or the sidebar nav.
